# Israel stock market API

Real-time Israel stock market API with WebSocket, OHLCV, and snapshot endpoints for TASE.

## Build With Israel Market Data

Kun Data provides a token-authenticated API stack for developers who need Israel market data in products that must load fast, stream live prices, and support charting or screening workflows.

This repository is focused on Israel coverage with support for TASE and is designed for teams building broker dashboards, watchlists, quant tools, stock screeners, mobile apps, and market overview pages.

## Why Developers Search For This

Developers usually need one or more of these:

- Real-time Israel stock market API
...
`Israel stock market API | TASE API | Israel stock data API | Israel OHLCV API`
...
### Does this support real-time Israel market data?
...
If you need Israel market data coverage for TASE, start with the product page and documentation:
